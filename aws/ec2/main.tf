...
terraform {
  backend "s3" {
    ...
    key            = "global/ec2/terraform.tfstate"     # Path to store state
...
resource "aws_instance" "srini1" {
  ...
  subnet_id     = "subnet-02875222072524e0a" #aws_subnet.main.id
  vpc_security_group_ids = ["sg-0d651c686d560ce78"]
...
#### MOVED to separate folder and added prevent destroy
